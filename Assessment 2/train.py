...
def run_episode(env, controller, is_agent):
    state = env.reset()
    for _ in range(EPISODE_SECONDS):
        d_state = env.discretized_state()
        if is_agent:
            action = controller.act(d_state)
        else:
...
def evaluate(agent):
    ...
    for ep in range(N_EVAL_EPISODES):
        ...
        info_a = run_episode(env_a, agent, is_agent=True)
    ...
    print("\n=== EVALUATION (20 unseen episodes, matched seeds) ===")
