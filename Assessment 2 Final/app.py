...
from analysis.model_loader import predict_volume, WEATHER_PRESETS, BASELINE_WEATHER
...
sim = {
    "env": IntersectionEnv(seed=random.randint(0, 100000), is_weekend=False, start_hour=8),
    ...
    "weather": "clouds",        # current weather preset, drives the live model
}


def make_weather_multiplier_fn():
    """Returns a callable(hour_float, is_weekend) -> multiplier that calls
    the trained regression model live, comparing the current weather preset
    against the baseline weather the static demand profile was built with.
    This is what makes the ML component do real work at runtime, driven by
    the dashboard's weather controls, instead of only running once offline.

    Caches by (weather, hour, is_weekend) since the same combination repeats
    every simulated second within the same hour — no need to hit the model
    twice for identical inputs."""
    cache = {}

    def fn(hour_float, is_weekend):
        hour = int(round(hour_float)) % 24
        key = (sim["weather"], hour, is_weekend)
        if key in cache:
            return cache[key]
        preset = WEATHER_PRESETS[sim["weather"]]
        day_of_week = 5 if is_weekend else 2  # representative day, matches generate_demand_profile.py
        predicted = predict_volume(hour=hour, day_of_week=day_of_week, month=6,
                                     is_weekend=int(is_weekend), is_holiday=0, **preset)
        baseline = predict_volume(hour=hour, day_of_week=day_of_week, month=6,
                                    is_weekend=int(is_weekend), is_holiday=0, **BASELINE_WEATHER)
        result = predicted / baseline if baseline > 0 else 1.0
        if len(cache) > 200:  # simple unbounded-growth guard, this will never realistically hit
            cache.clear()
        cache[key] = result
        return result
    return fn
...
sim["env"].live_weather_multiplier_fn = make_weather_multiplier_fn()
        ...
        with state_lock:
            if sim["running"]:
                ...
                if controller is agent:
                    action = agent.act_with_safety(env, greedy=False)
                else:
                ...
                payload = {
                    ...
                    "weather": sim["weather"],
                    "weather_multiplier": round(
                        env.live_weather_multiplier_fn(
                            ((env.t + env._day_offset_sec) / 3600.0) % 24.0, env.is_weekend
                        ) if env.live_weather_multiplier_fn else 1.0, 3
                    ),
                }
            else:
                payload = None
...
def reset_sim():
    with state_lock:
        sim["env"] = IntersectionEnv(seed=random.randint(0, 100000), is_weekend=False, start_hour=8)
        sim["env"].live_weather_multiplier_fn = make_weather_multiplier_fn()
...
def set_weather():
    name = request.json.get("weather")
    if name not in WEATHER_PRESETS:
        return jsonify({"error": "invalid weather preset", "valid": list(WEATHER_PRESETS.keys())}), 400
    with state_lock:
        sim["weather"] = name
    return jsonify({"ok": True, "weather": name})
